# gh deployment workflow

This is a solution to the [Github Deployment Workflow](https://roadmap.sh/projects/ggithub-actions-deployment-workflow) challenge from [roadmap.sh](https://roadmap.sh).

## Features
- Automated deployment to GitHub Pages using GitHub Actions.
- Simple HTML page as the content for the GitHub Pages site.

## Project URL
Find the project requirements here: https://roadmap.sh/projects/github-actions-deployment-workflow
